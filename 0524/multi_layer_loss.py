...
class MultiLayerLoss(nn.Module):
    ...
    def calculate_dloss(self, predictions, true_labels):
        ...
        for l in range(1, self.total_level):
            ...
            # 计算F1和F2分数
            tp = torch.sum((true_labels[l] == 0) & (current_lvl_pred == 0))
            fp = torch.sum((true_labels[l] == 1) & (current_lvl_pred == 0))
            fn = torch.sum((true_labels[l] == 1) & (current_lvl_pred == 1))
            ...
            # 将F1和F2分数纳入损失函数
            dloss += torch.sum(torch.pow(self.p_loss, D_l*l_prev)*torch.pow(self.p_loss, D_l*l_curr) - 1) - f1_weight * f1 - f2_weight * f2

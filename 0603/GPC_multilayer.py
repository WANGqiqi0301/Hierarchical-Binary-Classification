...
# 使用GPC训练模型
feature_map_nystroem = Nystroem(kernel='rbf', gamma=0.2, n_components=20)
kernel = 1.0 * RBF(1.0)
# 使用管道将Nystroem近似和GPC结合起来
models = [make_pipeline(feature_map_nystroem, GaussianProcessClassifier()) for _ in range(3)]
